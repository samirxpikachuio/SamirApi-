...
function makeRequest(endpoint, params = {}, method = 'GET') {
  return new Promise((resolve, reject) => {
    const url$1 = new url.URL(endpoint, BASE_URL);
    if (method === 'GET') {
      url$1.search = new URLSearchParams(params).toString();
    }
    const options = {
      method: method,
      headers: {
    ...
    const req = https__default["default"].request(url$1, options, (res) => {
      let data = '';
...
function makeImageRequest(endpoint, params = {}) {
  return new Promise((resolve, reject) => {
    const url$1 = new url.URL(endpoint, BASE_URL);
    url$1.search = new URLSearchParams(params).toString();
    https__default["default"].get(url$1, (res) => {
      if (res.statusCode >= 200 && res.statusCode < 300) {
        const chunks = [];
